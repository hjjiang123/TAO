/*
reseamble tcp flow and obtain the length of flows' data
*/
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
extern "C"
{
    ...
    void process_segment(flow_key_t *key, uint32_t seq, uint32_t len, uint8_t flags) {
    ...
    int plugin_0(struct rte_mbuf *pkt, char ****res, int rownum,int bucketnum,int bucketsize,int countersize, void *to_next[8])
    {
        ...
        if (ntohs(eth_hdr->ether_type) == RTE_ETHER_TYPE_IPV4)
        {
            ...
            if (ipv4_hdr->next_proto_id == IPPROTO_TCP)
            {
                ...
                process_segment(&key, ntohl(tcp_hdr->sent_seq), rte_pktmbuf_pkt_len(pkt) - sizeof(struct rte_ether_hdr) - sizeof(struct rte_ipv4_hdr) - sizeof(struct rte_tcp_hdr), tcp_hdr->tcp_flags);
